...
public class DeploymentUnitDir {
    ...
    public DocumentRegistry getDocRegistry() {
        if (_docRegistry == null) {
            ...
            DocumentRegistry tempDocRegistry = new DocumentRegistry();
            for (File file : wsdls) {
                URI uri = basedir.relativize(file.toURI());
                try {
                    tempDocRegistry.addDefinition((Definition4BPEL) r.readWSDL(new WSDLLocatorImpl(rf, uri)));
                } catch (WSDLException e) {
                    throw new ContextException("Couldn't read WSDL document at " + uri, e);
                }
            }
            if (getDeploymentDescriptor().getDeploy() != null) {
                List<Process> processList = getDeploymentDescriptor().getDeploy().getProcessList();
                if (processList != null) {
                    for (Process process : processList) {
                        if (process.getProvideList() != null) {
                            for (TProvide provide : process.getProvideList()) {
                                if (provide.getService() != null) {
                                    Definition4BPEL definition = tempDocRegistry.getDefinition(provide.getService().getName());
                                    if (definition != null) {
                                        _docRegistry.addDefinition(definition);
                                    }
                                }
                            }
                        }
                        if (process.getInvokeList() != null) {
                            for (TInvoke invoke : process.getInvokeList()) {
                                if (invoke.getService() != null) {
                                    Definition4BPEL definition = tempDocRegistry.getDefinition(invoke.getService().getName());
                                    if (definition != null) {
                                        _docRegistry.addDefinition(definition);
                                    }
                                }
