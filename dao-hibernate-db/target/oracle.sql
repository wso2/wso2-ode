...
create table STORE_PROCESS_PROP (propId varchar2(255 char) not null, value varchar2(2048 char), name varchar2(255 char) not null, primary key (propId, name));
create table STORE_VERSIONS (ID number(10,0) not null, VERSION number(19,0), primary key (ID));
create table TASK_ATTACHMENT (ID number(19,0) not null, ATTACHMENT_ID number(19,0), MESSAGE_EXCHANGE_ID number(19,0), INSERT_TIME timestamp, MLOCK number(10,0) not null, primary key (ID));
